...
ZelzalMeMe_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗔𝗯𝗼𝗱𝗧𝗵𝗼𝗻 ⌁ - اوامـر البصمـات 🎙](t.me/AbodThon) 𓆪\n\n"
    ...
    "\n 𓆩 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗔𝗯𝗼𝗱𝗧𝗵𝗼𝗻 ⌁](t.me/AbodThon) 𓆪"
)
...
async def on_meme_list(event):
    ...
    for filt in filters:
        if OUT_STR == "**⪼ لا يوجـد لديك بصمـات محفوظـه ❌**\n\n**⪼ ارسـل (** `.بصمه` **) + اسم البصمـه**\n**⪼بالـرد ع بصمـه او مقطـع صـوتـي 🔊**\n**⪼ لإضافتهـا لـ قائمـة بصماتك 🧾**":
            OUT_STR = "𓆩 𝗔𝗯𝗼𝗱𝗧𝗵𝗼𝗻 ⌁ - قائمـة بصمـاتك المضـافـة 🔊𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "🎙 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def on_meme_list(event):
    ...
    for filt in filters:
        if OUT_STR == "**⪼ لا يوجـد لديك ردود تلقائيـة لـ الخـاص ❌**\n\n**⪼ ارسـل (** `.اضف تلقائي` **) + كلمـة الـرد**\n**⪼بالـرد ع جملـة او ميديـا 🗣**\n**⪼ لإضافتهـا لـ قائمـة ردودك التلقائيـة 🧾**":
            OUT_STR = "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗔𝗯𝗼𝗱𝗧𝗵𝗼𝗻 ⌁ - ردودك التلقـائيـة خـاص 🗣𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "🎙 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
